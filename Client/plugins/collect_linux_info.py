...
def get_nic_info():
    ...
    :return:
    """
    nic_name_list = subprocess.Popen(" cat /proc/net/dev | awk  -F ':' '{if (NR >=3) print $1}'",
                                     stdout=subprocess.PIPE, shell=True).stdout.read().decode().split('\n')
    nic_list = []
    for nic_name in nic_name_list:
        if "lo" not in nic_name and nic_name != "":
            nic_dic = get_nic_ditail(nic_name)
